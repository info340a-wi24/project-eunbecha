...
function Table({ data, searchTerm, onSongSelect }) {
  const filteredData = data.filter(item => item.song.includes(searchTerm) || item.artist.includes(searchTerm));
...
//         <tbody>
//           {filteredData.map(item => (
//             <TableRow key={item.id} url={item.url} onClick={() => onSongSelect(item.url)} />
//             // <TableRow key={item.id} song={item.song} artist={item.artist} url={item.url} onClick={() => onSongSelect(item.url)} />
//           ))}
//         </tbody>
//       </table>
